...
#### A. Command Pattern
*   **Mối quan hệ:** **Implementation (`--|>`)** / UML mũi tên đứt vạch tam giác
    *   *Nơi dùng:* Các lớp `TrackByCodeCommand`, `ViewTrackingPageCommand`... trỏ về interface `OrderCommand`.
    *   *Sách định nghĩa:* Các Concrete Command chia sẻ chung giao tiếp khuôn mẫu `execute()`.
*   **Mối quan hệ:** **Dependency (`---->`)** (hoặc Association tùy góc nhìn cài đặt ẩn)
    *   *Nơi dùng:* `OrderTrackingController` gọi interface đai trà `OrderCommand` ở hàm giải quyết luồng.
    *   *Phân tích:* Khóa chặt Controller ở vai trò Invoker (Kẻ gọi). Nó chỉ ra lệnh "thử thi" request của form, không cần biết ruột bên trong Command thực chất lấy dữ liệu tra cứu ra sao.
